...
$error = '';
$success = '';
$deposit_data = ['amount' => '', 'method' => ''];
$withdrawal_data = ['amount' => '', 'method' => ''];
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_deposit'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        $deposit_data['method'] = filter_input(INPUT_POST, 'method', FILTER_SANITIZE_STRING);
        ...
        } elseif (!in_array($deposit_data['method'], $valid_methods)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_withdrawal'])) {
    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        $withdrawal_data['method'] = filter_input(INPUT_POST, 'method', FILTER_SANITIZE_STRING);

        // Validate inputs
        $valid_methods = ['USDT (TRC20)', 'Bitcoin (BTC)', 'Ethereum (ETH)', 'MPESA'];
        if ($user['referral_count'] < 3) {
            $error = 'You need at least 3 referrals to withdraw funds.';
        } elseif (!$withdrawal_data['amount'] || $withdrawal_data['amount'] <= 0) {
        ...
        } elseif (!in_array($withdrawal_data['method'], $valid_methods)) {
            ...
            try {
                ...
                $stmt->execute([$withdrawal_data['amount'], $user_id]);
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="p-4 bg-blue-50 rounded-lg">
                        <div class="flex items-center">
                            <i class="ri-wallet-line text-primary text-2xl mr-2"></i>
                            <h3 class="text-sm font-medium text-gray-600">Available Balance</h3>
                        </div>
                        <p class="text-2xl font-bold text-primary mt-2">$<?php echo number_format($balance['available_balance'], 2); ?></p>
                    </div>
                    <div class="p-4 bg-yellow-50 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-yellow-600 mt-2">$<?php echo number_format($balance['pending_balance'], 2); ?></p>
                    </div>
                    <div class="p-4 bg-green-50 rounded-lg">
                        ...
                        <p class="text-2xl font-bold text-green-600 mt-2">$<?php echo number_format($balance['total_withdrawn'], 2); ?></p>
                    </div>
                </div>
            </div>
        </section>
                ...
                <form method="POST" action="">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" min="10" step="0.01" placeholder="0.00" value="<?php echo htmlspecialchars($deposit_data['amount']); ?>" required class="block w-full pl-7 pr-12 py-2 border border-gray-300 rounded-button focus:ring-2 focus:ring-primary focus:ring-opacity-20 focus:outline-none text-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <select name="method" required class="block w-full py-2 pl-3 pr-8 border border-gray-300 rounded-button appearance-none focus:ring-2 focus:ring-primary focus:ring-opacity-20 focus:outline-none text-sm">
                                <option value="" disabled <?php echo $deposit_data['method'] === '' ? 'selected' : ''; ?>>Select a method</option>
                                <?php foreach (['USDT (TRC20)', 'Bitcoin (BTC)', 'Ethereum (ETH)', 'MPESA'] as $method): ?>
                                    <option value="<?php echo $method; ?>" <?php echo $deposit_data['method'] === $method ? 'selected' : ''; ?>><?php echo $method; ?></option>
                                <?php endforeach; ?>
                            </select>
                ...
                <form method="POST" action="">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="amount" min="10" step="0.01" placeholder="0.00" value="<?php echo htmlspecialchars($withdrawal_data['amount']); ?>" required class="block w-full pl-7 pr-12 py-2 border border-gray-300 rounded-button focus:ring-2 focus:ring-primary focus:ring-opacity-20 focus:outline-none text-sm">
                            <div class="absolute inset-y-0 right-0 flex items-center">
                                <button type="button" class="h-full px-3 text-xs text-primary font-medium bg-blue-50 rounded-r-button whitespace-nowrap" onclick="this.form.amount.value = <?php echo floor($balance['available_balance'] * 100) / 100; ?>">MAX</button>
                            </div>
                        </div>
                        <p class="mt-1 text-xs text-gray-500">Minimum withdrawal: $10.00. Referrals required: <?php echo $user['referral_count']; ?>/3</p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="relative">
                            <select name="method" required class="block w-full py-2 pl-3 pr-8 border border-gray-300 rounded-button appearance-none focus:ring-2 focus:ring-primary focus:ring-opacity-20 focus:outline-none text-sm">
                                <option value="" disabled <?php echo $withdrawal_data['method'] === '' ? 'selected' : ''; ?>>Select a method</option>
                                <?php foreach (['USDT (TRC20)', 'Bitcoin (BTC)', 'Ethereum (ETH)', 'MPESA'] as $method): ?>
                                    <option value="<?php echo $method; ?>" <?php echo $withdrawal_data['method'] === $method ? 'selected' : ''; ?>><?php echo $method; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none">
                                <i class="ri-arrow-down-s-line text-gray-400"></i>
